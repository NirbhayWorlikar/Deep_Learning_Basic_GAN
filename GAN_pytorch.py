...
def show_result(G_net, z_, num_epoch, show=False, save=False, path='result.png'):
    ...
    grid_size = 5
    fig, axis = plt.subplots(grid_size, grid_size, figsize=(5, 5))
    for a in range(5*5):
        b = a // 5
        c = a % 5
        axis[b, c].cla()
        axis[b, c].imshow(G_net(z_)[a, :].cpu().data.view(28, 28).numpy(), cmap='gray')
    plt.savefig(path)
...
if __name__ == '__main__':
    ...
    #train_data = datasets.MNIST(root=data_dir, train=True, transform=transform)
